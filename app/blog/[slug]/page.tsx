  ...
  return (
    <main className="min-h-dvh px-4 py-12 bg-[var(--ctp-mocha-base)] text-[var(--ctp-mocha-text)] relative">
      ...
      <article className="z-20 relative max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold text-[var(--ctp-mocha-lavender)] mb-4">{post.title}</h1>
        <time className="block text-[var(--ctp-mocha-subtext0)] mb-6">{post.published_at}</time>
        {post.image_url && (
          <img
            src={post.image_url}
            alt={post.title}
        ...
        {typeof post.content === 'string' && post.content.trim() !== '' ? (
          <div
            className="prose prose-invert prose-lg text-[var(--ctp-mocha-text)]"
            dangerouslySetInnerHTML={{ __html: post.content }}
          />
        ) : (
          <p className="text-[var(--ctp-mocha-subtext0)] italic">No content available.</p>
        )}
      </article>
    </main>
  );
